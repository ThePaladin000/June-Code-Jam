...
export function useSavedPlaces() {
 ...
 const saveUserPlaces = useCallback(async (placeId)=> {
    if (!isSignedIn || !user) {
    ...
    try {
        await userDataService.savePlace(user.id, placeId);

        //optimistically update state
        setSavedPlaces(prevPlaces => [...prevPlaces, placeId]);
  ...
  const removeUserPlaces = useCallback(async (placeId)=> {
    if (!isSignedIn || !user) {
  ...
  const isPlaceSaved = useCallback((placeId) => {
    ...
    return savedPlaces.some(place => place === placeId || place.id === placeId);
